...
public class JSONTokener {
   ...
   * tokener that throws {@code NullPointerExceptions} when methods are called.
   */
   ...
   * The opening brace '[' should have already been read. Note that "[]" yields
   * an empty array, but "[,]" returns a two-element array equivalent to
   * "[null,null]".
   */
   ...
   * input has been exhausted. The return value of this method is ambiguous for
   * JSON strings that contain the character '\0'.
   *
   * @return
   */
  public char next() {
  ...
  /**
   * Returns the next character that is not whitespace and does not belong to a
   * comment. If the input is exhausted before such a character can be found,
   * the null character '\0' is returned. The return value of this method is
   ...
   *
   * <p>
   * The returned string shares its backing character array with this tokener's
   * input string. If a reference to the returned string may be held
   * indefinitely, you should use {@code new String(result)} to copy it first to
   * avoid memory leaks.
   *
   * @param length
   * @return
   * @throws JSONException if the remaining input is not long enough to satisfy
   * this request.
   */
  ...
  /**
   * Returns the {@link String#trim trimmed} string holding the characters up to
   * but not including the first of:
   * <ul>
   ...
   * {@code to}. If the remaining input doesn't contain {@code to}, the input is
   * unchanged.
   *
   * @param to
   * @return
