 ...
 * matching call to {@link #endArray} and every call to {@link
...
public class JSONStringer {
   ...
   * next element.
   */
  public static final String NONEMPTY_ARRAY = "NONEMPTY_ARRAY";

  /**
   * An object with no keys or values requires no separators or newlines before
   * it is closed.
   */
  public static final String EMPTY_OBJECT = "EMPTY_OBJECT";

  /**
   * An object whose most recent element is a key. The next element must be a
   * value.
   */
  public static final String DANGLING_KEY = "DANGLING_KEY";

  /**
   * An object with at least one name/value pair requires a comma and newline
   * before the next element.
   */
  public static final String NONEMPTY_OBJECT = "NONEMPTY_OBJECT";
  ...
  /**
   * Begins encoding a new object. Each call to this method must be paired with
   * a call to {@link #endObject}.
   *
   * @return this stringer.
   */
   ...
   * bracket.
   */
  JSONStringer open(String empty, String openBracket) throws JSONException {
   ...
   * given bracket.
   */
  JSONStringer close(String empty, String nonempty, String closeBracket) throws JSONException {
    String context = peek();
   ...
   */
  private String peek() throws JSONException {
    if (stack.isEmpty()) {
      throw new JSONException("Nesting problem");
    }
   ...
   */
  private void replaceTop(String topOfStack) {
   ...
   * Integer, Long, Double or null. May not be {@link Double#isNaN() NaNs} or
  ...
  public JSONStringer value(double value) throws JSONException {
    ...
    beforeValue();
    out.append(JSONObject.numberToString(new Double(value)));
    return this;
  }
  ...
  private static StringBuffer printString(String value, StringBuffer out) {
    for (int i = 0, length = value.length(); i < length; i++) {
      ...
      switch (c) {
        ...
        default:
          if (c <= 0x1F) {
            out.append("\\u" + Integer.toHexString(0x10000 + c).substring(1));
          } else {
            out.append(c);
          }
          break;
   ...
   * the stack to expect the key's value.
   */
   ...
   * {@link Object#toString}, this method returns null if the stringer contains
   * no data.
   */
