 ...
 * {@link edu.xtec.util.db.ConnectionBean} objects. Based on DbConnectionBroker
 * 1.0.13, by Marc A. Mnich
 *
 * @author fbusquets
 * @version 1.0.13
 */
public class PooledConnectionBeanProvider extends ConnectionBeanProvider implements Runnable {
    ...
    /**
     * Key for absolute path name for log file. e.g. 'c:/temp/mylog.log'.
     * Default is 'pooledConnectionBean.log' in user's dir
     */
    public static final String LOG_FILE = "dbLogFile";
    private static final String DEFAULT_LOG_FILE = "connectionPool.log";
    /**
     * Key for time in days between connection resets. (Reset does a basic
     * cleanup). Default is 1.0
     */
    ...
    /**
     * Key for max time a connection can be checked out before being recycled.
    ...
    /**
     * Key for level of debug messages output to the log file. 0 -> no messages,
     * 1 -> Errors, 2 -> Warnings, 3 -> Information. Default is 2.
     */
    ...
    private String pid;
    /**
     * Number of times this ConnectionBean provider has been used since its
     * creation.
     */
    ...
    public void run() {
        ...
        while (forever) {
             ...
             *
             * FileReader fr=null; BufferedReader in=null; // Make sure the log
             * file is the one this instance opened // If not, clean it up! try
             * { fr=new FileReader(logPIDFileString); in = new
             * BufferedReader(fr); String curr_pid = in.readLine();
             * if(curr_pid.equals(pid)) { //log.println("They match = " +
             * curr_pid); } else { //log.println("No match = " + curr_pid);
             * if(log!=null) log.close();
             *
             * // Close all connections silently - they are definitely dead.
             * for(int i=0; i < currConnections; i++) { try {
             * connPool[i].closeConnection(); } catch (SQLException e1) {} //
             * ignore } // Returning from the run() method kills the thread
             * return; }
             *
             * // moved to "finally" //in.close();
             *
             * } catch (IOException e1) { if(log!=null){ log.print(new
             * Date().toString() + "Can't read the file for pid info: " +
             * logPIDFileString + " - "); log.println(e1.getMessage()); } }
             * finally{ try{ if(in!=null) in.close(); if(fr!=null) fr.close();
             * in=null; fr=null; } catch(IOException ex){ // ignore exceptions
             * on close } }
             */
     ...
     * a null.
     *
     * @return The ConnectionBean object, ready to be used. Remember to free it
     * using freeConnectionBean, as explained in {@link
     ...
     */
    /**
     * Multi-phase shutdown. having following sequence: <OL> <LI><code>getConnection()</code>
     * will refuse to return connections. <LI>The housekeeping thread is shut
     * down.<br> Up to the time of
     * <code>millis</code> milliseconds after shutdown of the housekeeping
     * thread,
     * <code>freeConnection()</code> can still be called to return used
     * connections. <LI>After
     * <code>millis</code> milliseconds after the shutdown of the housekeeping
     * thread, all connections in the pool are closed. <LI>If any connections
     * were in use while being closed then a
     * <code>SQLException</code> is thrown. <LI>The log is closed. </OL><br>
     ...
     * the
     * <code>destroy()</code> method with a
     * <code>millis</code> value of 10000 (10 seconds) and ignores
     * <code>SQLException</code> thrown by that method.
     *
     * @see #destroy(int)
     */
