 ...
 */
 ...
 if (isset($_GET['path'])) {
     ...
     case 'act_es.jsp':
         ...
         $url = 'https://projectes.xtec.cat/clic/'.$lang.'/repo/?prj='.getPathFromId($id);
     ...
     case 'listact_es.jsp':
         ...
         $url = 'https://projectes.xtec.cat/clic/'.$lang.'/repo/?'
         ...
         // Combina títol, autor i descripció en un únic paràmetre "text"
         $txt = buildListParam('text_titol', 'x').' '.buildListParam('text_aut', 'x').' '.buildListParam('text_desc', 'x');
         $txt = trim(str_replace('&x=', '', $txt));
         if($txt<>'')
           $url = $url.'&text="'.$txt.'"';
