...
tinyMCE.importThemeLanguagePack('qv');

// Variable declarations
var TinyMCE_qv_autoImportCSSClasses = true;
var TinyMCE_qv_resizer = new Object();
var TinyMCE_qv_buttons = [
 ...
 */
function TinyMCE_qv_getControlHTML(button_name)
{
	...
	for (var i=0; i<TinyMCE_qv_buttons.length; i++)
	{
		var but = TinyMCE_qv_buttons[i];
	...
	switch (button_name)
	{
		case "formatselect":
			...
			var formats = tinyMCE.getParam("theme_qv_blockformats", "p,address,pre,h1,h2,h3,h4,h5,h6", true).split(',');
			var lookup = [
		...
		case "fontselect":
			...
			var fonts = tinyMCE.getParam("theme_qv_fonts", nFonts).split(';');
 ...
 */
function TinyMCE_qv_execCommand(editor_id, element, command, user_interface, value)
{
	switch (command)
	{
		case "mceForeColor":
		...
		case "mceBackColor":
			...
			template['width'] += tinyMCE.getLang('lang_theme_qv_backcolor_delta_width', 0);
			template['height'] += tinyMCE.getLang('lang_theme_qv_backcolor_delta_height', 0);
		...
		case "mceColorPicker":
			if (user_interface) {
				...
				template['width'] += tinyMCE.getLang('lang_theme_qv_colorpicker_delta_width', 0);
				template['height'] += tinyMCE.getLang('lang_theme_qv_colorpicker_delta_height', 0);
		...
		case "mceCodeEditor":
			...
			template['width'] = parseInt(tinyMCE.getParam("theme_qv_source_editor_width", 500));
			template['height'] = parseInt(tinyMCE.getParam("theme_qv_source_editor_height", 400));
		...
		case "mceCharMap":
			...
			template['width'] += tinyMCE.getLang('lang_theme_qv_charmap_delta_width', 0);
			template['height'] += tinyMCE.getLang('lang_theme_qv_charmap_delta_height', 0);
		...
		case "mceInsertAnchor":
			...
			template['width'] += tinyMCE.getLang('lang_theme_qv_anchor_delta_width', 0);
			template['height'] += tinyMCE.getLang('lang_theme_qv_anchor_delta_height', 0);
 ...
 */
function TinyMCE_qv_getEditorTemplate(settings, editorId)
{
	...
	var resizing = tinyMCE.getParam("theme_qv_resizing", false);
	var path = tinyMCE.getParam("theme_qv_path", true);
	var statusbarHTML = '<div id="{$editor_id}_path" class="mceStatusbarPathText" style="display: ' + (path ? "block" : "none") + '">&nbsp;</div><div id="{$editor_id}_resize" class="mceStatusbarResize" style="display: ' + (resizing ? "block" : "none") + '" onmousedown="TinyMCE_qv_setResizing(event,\'{$editor_id}\',true);"></div><br style="clear: both" />';
	var layoutManager = tinyMCE.getParam("theme_qv_layout_manager", "SimpleLayout");

	// Setup style select options -- MOVED UP FOR EXTERNAL TOOLBAR COMPATABILITY!
	var styleSelectHTML = '<option value="">{$lang_theme_style_select}</option>';
	if (settings['theme_qv_styles'])
	{
		var stylesAr = settings['theme_qv_styles'].split(';');
		...
		TinyMCE_qv_autoImportCSSClasses = false;
	}
	...
	switch(layoutManager)
	{
		case "SimpleLayout" : //the default TinyMCE Layout (for backwards compatibility)...
			var toolbarHTML = "";
			var toolbarLocation = tinyMCE.getParam("theme_qv_toolbar_location", "bottom");
			var toolbarAlign = tinyMCE.getParam("theme_qv_toolbar_align", "center");
			var pathLocation = tinyMCE.getParam("theme_qv_path_location", "none"); // Compatiblity
			var statusbarLocation = tinyMCE.getParam("theme_qv_statusbar_location", pathLocation);
			var defVals = {
				//theme_qv_buttons1 : "bold,italic,underline,strikethrough,separator,justifyleft,justifycenter,justifyright,justifyfull,separator,styleselect,formatselect",
				theme_qv_buttons1 : "bold,italic,underline,strikethrough,separator,justifyleft,justifycenter,justifyright,justifyfull,separator,styleselect,formatselect",
				theme_qv_buttons2 : "bullist,numlist,separator,outdent,indent,separator,undo,redo,separator,link,unlink,anchor,image,cleanup,help,code",
				theme_qv_buttons3 : "hr,removeformat,visualaid,separator,sub,sup,separator,charmap"
			};
			...
			// Render rows
			for (var i=1; i<100; i++) {
				var def = defVals["theme_qv_buttons" + i];

				buttons = tinyMCE.getParam("theme_qv_buttons" + i, def == null ? '' : def, true, ',');
				if (buttons.length == 0)
					break;

				buttons = removeFromArray(buttons, tinyMCE.getParam("theme_qv_disable", "", true, ','));
				buttons = addToArray(buttons, tinyMCE.getParam("theme_qv_buttons" + i + "_add", "", true, ','));
				buttons = addToArray(tinyMCE.getParam("theme_qv_buttons" + i + "_add_before", "", true, ','), buttons);
		...
		case "RowLayout" : //Container Layout - containers defined in "theme_qv_containers" are rendered from top to bottom.
			template['html'] = '<table class="mceEditor" border="0" cellpadding="0" cellspacing="0" width="{$width}" height="{$height}" style="width:{$width}px;height:{$height}px"><tbody>';

			var containers = tinyMCE.getParam("theme_qv_containers", "", true, ",");
			var defaultContainerCSS = tinyMCE.getParam("theme_qv_containers_default_class", "container");
			var defaultContainerAlign = tinyMCE.getParam("theme_qv_containers_default_align", "center");
			...
			for (var i = 0; i < containers.length; i++)
			{
				...
				{
					var curContainer = tinyMCE.getParam("theme_qv_container_"+containers[i], "", true, ',');
					var curContainerHTML = "";
					var curAlign = tinyMCE.getParam("theme_qv_container_"+containers[i]+"_align", defaultContainerAlign);
					var curCSS = tinyMCE.getParam("theme_qv_container_"+containers[i]+"_class", defaultContainerCSS);
		...
		case "CustomLayout" : //User defined layout callback...
			var customLayout = tinyMCE.getParam("theme_qv_custom_layout","");
 ...
 */
function TinyMCE_qv_setResizing(e, editor_id, state) {
	e = typeof(e) == "undefined" ? window.event : e;

	var resizer = TinyMCE_qv_resizer;
	...
	if (state) {
		...
		if (!resizer.eventHandlers) {
			if (tinyMCE.isMSIE)
				tinyMCE.addEvent(document, "mousemove", TinyMCE_qv_resizeEventHandler);
			else
				tinyMCE.addEvent(window, "mousemove", TinyMCE_qv_resizeEventHandler);

			tinyMCE.addEvent(document, "mouseup", TinyMCE_qv_resizeEventHandler);
		...
		resizer.horizontal = tinyMCE.getParam("theme_qv_resize_horizontal", true);
	} else {
...
function TinyMCE_qv_initInstance(inst) {
	if (tinyMCE.getParam("theme_qv_resizing", false)) {
		var w = TinyMCE_qv_getCookie("TinyMCE_" + inst.editorId + "_width");
		var h = TinyMCE_qv_getCookie("TinyMCE_" + inst.editorId + "_height");

		TinyMCE_qv_resizeTo(inst, w, h, tinyMCE.getParam("theme_qv_resize_horizontal", true));
	}
}

function TinyMCE_qv_setCookie(name, value, expires, path, domain, secure) {
...
function TinyMCE_qv_getCookie(name) {
...
function TinyMCE_qv_resizeTo(inst, w, h, set_w) {
 ...
 */
function TinyMCE_qv_resizeEventHandler(e) {
	var resizer = TinyMCE_qv_resizer;
	...
	switch (e.type) {
		...
		case "mouseup":
			TinyMCE_qv_setResizing(e, editorId, false);
			TinyMCE_qv_resizeTo(tinyMCE.getInstanceById(editorId), resizer.width + dx, resizer.height + dy, resizer.horizontal);
			...
			// Set the cookies
			TinyMCE_qv_setCookie("TinyMCE_" + editorId + "_width", "" + (resizer.horizontal ? resizer.width + dx : ""), expires);
			TinyMCE_qv_setCookie("TinyMCE_" + editorId + "_height", "" + (resizer.height + dy), expires);
			break;
	}
}
 ...
 */
function TinyMCE_qv_getInsertLinkTemplate()
{
	var template = new Array();
 ...
 */
function TinyMCE_qv_getInsertImageTemplate()
{
	var template = new Array();
 ...
 * Node change handler.
 */
function TinyMCE_qv_handleNodeChange (editor_id, node, undo_index,
															  undo_levels, visual_aid, any_selection)
{
	...
	if (selectElm)
	{
		TinyMCE_qv_setupCSSClasses(editor_id);
...
function TinyMCE_qv_setupCSSClasses(editor_id)
{
	if (!TinyMCE_qv_autoImportCSSClasses)
	{
		return;
	}
