...
tinyMCE.importThemeLanguagePack('educampus');

// Variable declarations
var TinyMCE_educampus_autoImportCSSClasses = true;
var TinyMCE_educampus_resizer = new Object();
var TinyMCE_educampus_buttons = [
 ...
 */
function TinyMCE_educampus_getControlHTML(button_name)
{
	...
	for (var i=0; i<TinyMCE_educampus_buttons.length; i++)
	{
		var but = TinyMCE_educampus_buttons[i];
	...
	switch (button_name)
	{
		case "formatselect":
			...
			var formats = tinyMCE.getParam("theme_educampus_blockformats", "p,address,pre,h1,h2,h3,h4,h5,h6", true).split(',');
			var lookup = [
		...
		case "styleselect":
			//styleselect
		return '<select id="{$editor_id}_styleSelect" onmousedown="TinyMCE_educampus_setupCSSClasses(\'{$editor_id}\');" name="{$editor_id}_styleSelect" onfocus="tinyMCE.addSelectAccessibility(event, this, window);" onchange="tinyMCE.execInstanceCommand(\'{$editor_id}\',\'mceSetCSSClass\',false,this.options[this.selectedIndex].value);" class="mceSelectList">{$style_select_options}</select>';
		...
		case "fontselect":
			...
			var fonts = tinyMCE.getParam("theme_educampus_fonts", nFonts).split(';');
 ...
 */
function TinyMCE_educampus_execCommand(editor_id, element, command, user_interface, value)
{
	switch (command)
	{
		case "mceForeColor":
		...
		case "mceBackColor":
			...
			template['width'] += tinyMCE.getLang('lang_theme_educampus_backcolor_delta_width', 0);
			template['height'] += tinyMCE.getLang('lang_theme_educampus_backcolor_delta_height', 0);
		...
		case "mceColorPicker":
			if (user_interface) {
				...
				template['width'] += tinyMCE.getLang('lang_theme_educampus_colorpicker_delta_width', 0);
				template['height'] += tinyMCE.getLang('lang_theme_educampus_colorpicker_delta_height', 0);
		...
		case "mceCodeEditor":
			...
			template['width'] = parseInt(tinyMCE.getParam("theme_educampus_source_editor_width", 500));
			template['height'] = parseInt(tinyMCE.getParam("theme_educampus_source_editor_height", 400));
		...
		case "mceCharMap":
			...
			template['width'] += tinyMCE.getLang('lang_theme_educampus_charmap_delta_width', 0);
			template['height'] += tinyMCE.getLang('lang_theme_educampus_charmap_delta_height', 0);
		...
		case "mceInsertAnchor":
			...
			template['width'] += tinyMCE.getLang('lang_theme_educampus_anchor_delta_width', 0);
			template['height'] += tinyMCE.getLang('lang_theme_educampus_anchor_delta_height', 0);
 ...
 */
function TinyMCE_educampus_getEditorTemplate(settings, editorId)
{
	...
	var resizing = tinyMCE.getParam("theme_educampus_resizing", false);
	var path = tinyMCE.getParam("theme_educampus_path", true);
	var statusbarHTML = '<div id="{$editor_id}_path" class="mceStatusbarPathText" style="display: ' + (path ? "block" : "none") + '">&nbsp;</div><div id="{$editor_id}_resize" class="mceStatusbarResize" style="display: ' + (resizing ? "block" : "none") + '" onmousedown="TinyMCE_educampus_setResizing(event,\'{$editor_id}\',true);"></div><br style="clear: both" />';
	var layoutManager = tinyMCE.getParam("theme_educampus_layout_manager", "SimpleLayout");

	// Setup style select options -- MOVED UP FOR EXTERNAL TOOLBAR COMPATABILITY!
	var styleSelectHTML = '<option value="">{$lang_theme_style_select}</option>';
	if (settings['theme_educampus_styles'])
	{
		var stylesAr = settings['theme_educampus_styles'].split(';');
		...
		TinyMCE_educampus_autoImportCSSClasses = false;
	}
	...
	switch(layoutManager)
	{
		case "SimpleLayout" : //the default TinyMCE Layout (for backwards compatibility)...
			var toolbarHTML = "";
			var toolbarLocation = tinyMCE.getParam("theme_educampus_toolbar_location", "bottom");
			var toolbarAlign = tinyMCE.getParam("theme_educampus_toolbar_align", "center");
			var pathLocation = tinyMCE.getParam("theme_educampus_path_location", "none"); // Compatiblity
			var statusbarLocation = tinyMCE.getParam("theme_educampus_statusbar_location", pathLocation);
			var defVals = {
				//theme_educampus_buttons1 : "bold,italic,underline,strikethrough,separator,justifyleft,justifycenter,justifyright,justifyfull,separator,styleselect,formatselect",
				theme_educampus_buttons1 : "bold,italic,underline,strikethrough,separator,justifyleft,justifycenter,justifyright,justifyfull,separator,styleselect,formatselect",
				theme_educampus_buttons2 : "bullist,numlist,separator,outdent,indent,separator,undo,redo,separator,link,unlink,anchor,image,cleanup,help,code",
				theme_educampus_buttons3 : "hr,removeformat,visualaid,separator,sub,sup,separator,charmap"
			};
			...
			// Render rows
			for (var i=1; i<100; i++) {
				var def = defVals["theme_educampus_buttons" + i];

				buttons = tinyMCE.getParam("theme_educampus_buttons" + i, def == null ? '' : def, true, ',');
				if (buttons.length == 0)
					break;

				buttons = removeFromArray(buttons, tinyMCE.getParam("theme_educampus_disable", "", true, ','));
				buttons = addToArray(buttons, tinyMCE.getParam("theme_educampus_buttons" + i + "_add", "", true, ','));
				buttons = addToArray(tinyMCE.getParam("theme_educampus_buttons" + i + "_add_before", "", true, ','), buttons);
		...
		case "RowLayout" : //Container Layout - containers defined in "theme_educampus_containers" are rendered from top to bottom.
			template['html'] = '<table class="mceEditor" border="0" cellpadding="0" cellspacing="0" width="{$width}" height="{$height}" style="width:{$width}px;height:{$height}px"><tbody>';

			var containers = tinyMCE.getParam("theme_educampus_containers", "", true, ",");
			var defaultContainerCSS = tinyMCE.getParam("theme_educampus_containers_default_class", "container");
			var defaultContainerAlign = tinyMCE.getParam("theme_educampus_containers_default_align", "center");
			...
			for (var i = 0; i < containers.length; i++)
			{
				...
				{
					var curContainer = tinyMCE.getParam("theme_educampus_container_"+containers[i], "", true, ',');
					var curContainerHTML = "";
					var curAlign = tinyMCE.getParam("theme_educampus_container_"+containers[i]+"_align", defaultContainerAlign);
					var curCSS = tinyMCE.getParam("theme_educampus_container_"+containers[i]+"_class", defaultContainerCSS);
		...
		case "CustomLayout" : //User defined layout callback...
			var customLayout = tinyMCE.getParam("theme_educampus_custom_layout","");
 ...
 */
function TinyMCE_educampus_setResizing(e, editor_id, state) {
	e = typeof(e) == "undefined" ? window.event : e;

	var resizer = TinyMCE_educampus_resizer;
	...
	if (state) {
		...
		if (!resizer.eventHandlers) {
			if (tinyMCE.isMSIE)
				tinyMCE.addEvent(document, "mousemove", TinyMCE_educampus_resizeEventHandler);
			else
				tinyMCE.addEvent(window, "mousemove", TinyMCE_educampus_resizeEventHandler);

			tinyMCE.addEvent(document, "mouseup", TinyMCE_educampus_resizeEventHandler);
		...
		resizer.horizontal = tinyMCE.getParam("theme_educampus_resize_horizontal", true);
	} else {
...
function TinyMCE_educampus_initInstance(inst) {
	if (tinyMCE.getParam("theme_educampus_resizing", false)) {
		var w = TinyMCE_educampus_getCookie("TinyMCE_" + inst.editorId + "_width");
		var h = TinyMCE_educampus_getCookie("TinyMCE_" + inst.editorId + "_height");

		TinyMCE_educampus_resizeTo(inst, w, h, tinyMCE.getParam("theme_educampus_resize_horizontal", true));
	}
}

function TinyMCE_educampus_setCookie(name, value, expires, path, domain, secure) {
...
function TinyMCE_educampus_getCookie(name) {
...
function TinyMCE_educampus_resizeTo(inst, w, h, set_w) {
 ...
 */
function TinyMCE_educampus_resizeEventHandler(e) {
	var resizer = TinyMCE_educampus_resizer;
	...
	switch (e.type) {
		...
		case "mouseup":
			TinyMCE_educampus_setResizing(e, editorId, false);
			TinyMCE_educampus_resizeTo(tinyMCE.getInstanceById(editorId), resizer.width + dx, resizer.height + dy, resizer.horizontal);
			...
			// Set the cookies
			TinyMCE_educampus_setCookie("TinyMCE_" + editorId + "_width", "" + (resizer.horizontal ? resizer.width + dx : ""), expires);
			TinyMCE_educampus_setCookie("TinyMCE_" + editorId + "_height", "" + (resizer.height + dy), expires);
			break;
	}
}
 ...
 */
function TinyMCE_educampus_getInsertLinkTemplate()
{
	var template = new Array();
 ...
 */
function TinyMCE_educampus_getInsertImageTemplate()
{
	var template = new Array();
 ...
 * Node change handler.
 */
function TinyMCE_educampus_handleNodeChange (editor_id, node, undo_index,
															  undo_levels, visual_aid, any_selection)
{
	...
	if (selectElm)
	{
		TinyMCE_educampus_setupCSSClasses(editor_id);
...
function TinyMCE_educampus_setupCSSClasses(editor_id)
{
	if (!TinyMCE_educampus_autoImportCSSClasses)
	{
		return;
	}
